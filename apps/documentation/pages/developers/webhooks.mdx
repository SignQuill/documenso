...
- Optionally, you can provide a secret key that will be used to sign the payload. This key will be included in the `X-{APP_NAME}-Secret` header of the request (where `{APP_NAME}` is your application name, e.g., `X-Documenso-Secret`).
